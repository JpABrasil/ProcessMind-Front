...
function returnChat(agente: string, id_chat: string,usuario: string,setChatAtual:any) {
  ...
  .then((data) => {
      const chatArray: ChatMsg[] = []
      data.historico.forEach((message: { role: string; parts: any }) => {
        if (!message || !message.role || !message.parts) {
          // Se a mensagem for nula ou não tiver role ou parts, continue para a próxima
          return;
        }
        const role = message.role
        const parts = message.parts
        const text = parts[0].text //Teoricamente, o texto é o primeiro elemento do array parts -> analisar isso depois
...
export function AppSidebar({ agente, usuario}: AppSidebarProps) {
  const { setChatAtual } = useChatContext();
  const [chats, setChats] = useState<SidebarItem[]>([]); // Estado para armazenar os chats
  ...
  function loadChats(usuario: string,agente: string) {
    ...
    .then((data) => {
        const chatsData = data.chats.map((chat:{ name: string }) => ({
            title: chat,  // O nome do chat é atribuído ao campo title
        }))
        setChats(chatsData)
    })
    .catch((error) => {
  ...
  function newChat() {
    setChatAtual([]); // Limpa o chat atual
    const newChatItem: SidebarItem = {
        title: "Novo Chat ", // Gera um nome temporário com base no horário atual
    };

    //Checar se já existe um chat com o mesmo nome
    const chatExists = chats.some((chat) => chat.title === newChatItem.title);
    if (chatExists) {
      return; // Se já existir, não faz nada
    }

    // Atualiza o estado de chats com o novo item
    setChats((prevChats: SidebarItem[]) => [...prevChats, newChatItem]);
    ...
    <Sidebar style={{position:"relative"}}>
      <SidebarContent className="bg-green-900 " >
        <SidebarGroup className="pt-2">
          <SidebarGroupLabel className="text-2xl text-white justify-between">
            Chats
            <button onClick={newChat}className="hover:cursor-pointer">
